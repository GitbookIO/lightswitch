...
```sh
npm install git+ssh://git@github.com:stainless-sdks/lightswitch-typescript.git
```
...
async function main() {
  const projects: Lightswitch.ProjectListResponse = await client.projects.list();
}
...
async function main() {
  const projects = await client.projects.list().catch(async (err) => {
    if (err instanceof Lightswitch.APIError) {
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/lightswitch-typescript/issues) with questions, bugs, or suggestions.
