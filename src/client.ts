...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['LIGHTSWITCH_BASE_URL'].
   */
...
export class Lightswitch {
   ...
   * API Client for interfacing with the Lightswitch API.
   *
   * @param {string} [opts.baseURL=process.env['LIGHTSWITCH_BASE_URL'] ?? https://lightswitch.gitbook.dev/api] - Override the default base URL for the API.
   ...
   */
  constructor({ baseURL = readEnv('LIGHTSWITCH_BASE_URL'), ...opts }: ClientOptions = {}) {
    const options: ClientOptions = {
      ...opts,
      baseURL: baseURL || `https://lightswitch.gitbook.dev/api`,
    };
  ...
  protected stringifyQuery(query: Record<string, unknown>): string {
    return Object.entries(query)
      ...
      .map(([key, value]) => {
        ...
        throw new Errors.LightswitchError(
  ...
  static Lightswitch = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static LightswitchError = Errors.LightswitchError;
