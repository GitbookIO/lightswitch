...
import { APIPromise } from 'lightswitch-api/core/api-promise';

import util from 'node:util';
import Lightswitch from 'lightswitch-api';
import { APIUserAbortError } from 'lightswitch-api';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['LIGHTSWITCH_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Lightswitch) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Lightswitch({ logger: logger, logLevel: 'debug' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Lightswitch({ logger: logger, logLevel: 'info' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['LIGHTSWITCH_LOG'] = 'debug';
      const client = new Lightswitch({ logger: logger });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['LIGHTSWITCH_LOG'] = 'not a log level';
      const client = new Lightswitch({ logger: logger });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'LIGHTSWITCH_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['LIGHTSWITCH_LOG'] = 'debug';
      const client = new Lightswitch({ logger: logger, logLevel: 'off' });
  ...
  describe('defaultQuery', () => {
    ...
    test('overriding with `undefined`', () => {
      const client = new Lightswitch({ baseURL: 'http://localhost:5000/', defaultQuery: { hello: 'world' } });
  ...
  test('explicit global fetch', async () => {
    ...
    const client = new Lightswitch({ baseURL: 'http://localhost:5000/', fetch: defaultFetch });
  });
  ...
  test('normalized method', async () => {
    ...
    const client = new Lightswitch({ baseURL: 'http://localhost:5000/', fetch: testFetch });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Lightswitch({ baseURL: 'http://localhost:5000/custom/path/' });
    ...
    afterEach(() => {
      process.env['LIGHTSWITCH_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Lightswitch({ baseURL: 'https://example.com' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['LIGHTSWITCH_BASE_URL'] = 'https://example.com/from_env';
      const client = new Lightswitch({});
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['LIGHTSWITCH_BASE_URL'] = ''; // empty
      const client = new Lightswitch({});
      expect(client.baseURL).toEqual('https://lightswitch.gitbook.dev/api');
    });

    test('blank env variable', () => {
      process.env['LIGHTSWITCH_BASE_URL'] = '  '; // blank
      const client = new Lightswitch({});
      expect(client.baseURL).toEqual('https://lightswitch.gitbook.dev/api');
    });
  });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Lightswitch({ maxRetries: 4 });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Lightswitch({});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Lightswitch({ timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Lightswitch({ fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Lightswitch({ fetch: testFetch });
